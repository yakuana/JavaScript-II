...
ticketPriceTotal = runners.reduce((donation, runner) => {return donation += runner.donation}, 0); 
...
// Problem 1 - Create a list of every runner's email. 
...
// Problem 2 - Find all company names that start with J. 

let companyPop = []; 

companyPop = runners.filter((runner) => runner.company_name[0] === "J"); 

console.log(companyPop); 

// Problem 3 - Take 5% from each donation to pay for security gaurds. Given a 200 dollar down payment 
// return the toal alloted money for security. 

let fee = runners.reduce((total, runner) => {return total += runner.donation * .05}, 200); 

// toFixed gives the number of decimals after a digit rounded up 
